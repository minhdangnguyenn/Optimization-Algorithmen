  ...
  return (
    <div className="visualization">
        ...
        {result.boxes.map((box: Box, boxIndex: number) => (
          <div key={boxIndex} style={{ marginBottom: '20px' }}>
            ...
            >
              {box.rectangles.map((rect, rectIndex) => {
                ...
                return (
                  ...
                  >
                    <div>R{rectIndex + 1}</div>
                    <div>{rect.width}×{rect.height}</div>
                  </div>
                );
              })}
            </div>
            ...
            <div style={{ fontSize: '12px', color: '#666', margin: '5px' }}>
              ...
              Utilization: {box.utilization.toFixed(2)}%
            </div>
          </div>
        ))}
      </div>
      ...
      {result.boxes.length === 0 && (
        <p>No boxes created - all rectangles might be too large for the box size.</p>
      )}
    </div>
  );
};